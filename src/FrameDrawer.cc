...
FrameDrawer::FrameDrawer(Atlas* pAtlas, int nCam):both(false),mpAtlas(pAtlas)
{
    mState=Tracking::SYSTEM_NOT_READY;
    mvIm.resize(nCam, cv::Mat(480,640,CV_8UC3, cv::Scalar(0,0,0)));
}

std::vector<cv::Mat> FrameDrawer::DrawFrame(float imageScale)
{
    std::vector<cv::Mat> vim(mvIm.size(), cv::Mat(480,641,CV_8UC3, cv::Scalar(0,0,0)));
    ...
    //Copy variables within scoped mutex
    {
        ...
        for (int c = 0; c < mvIm.size(); ++c)
        {
            if (id == 0 && c < mvIm.size()-1) continue;
            mvIm[c].copyTo(vim[c]);
        }
    ...
    if(imageScale != 1.f)
    {
        for (int c = 0; c < vim.size(); ++c)
        {
            int imWidth = vim[c].cols / imageScale;
            int imHeight = vim[c].rows / imageScale;
            cv::resize(vim[c], vim[c], cv::Size(imWidth, imHeight));
        }
    }
    ...
    {
        if (id == 0 && c < mvIm.size()-1) continue;
        if(vim[c].channels()<3) //this should be always true
            cvtColor(vim[c],vim[c],cv::COLOR_GRAY2BGR);
    }
    ...
    if(state==Tracking::NOT_INITIALIZED)
    {
        for(unsigned int i=0; i<vMatches.size(); i++)
        {
            if(vMatches[i]>=0)
            {
                ...
                cv::line(vim.back(),pt1,pt2,standardColor);
            }
        }
        for(vector<pair<cv::Point2f, cv::Point2f> >::iterator it=vTracks.begin(); it!=vTracks.end(); it++)
        {
            ...
            cv::line(vim.back(),pt1,pt2, standardColor,5);
        }
    ...
    else if(state==Tracking::OK) //TRACKING
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(vbVO[i] || vbMap[i])
            {
                ...
                if(vbMap[i])
                {
                    cv::rectangle(vim[c],pt1,pt2,standardColor);
                    cv::circle(vim[c],point,2,standardColor,-1);
                    mnTracked++;
                }
                else // This is match to a "visual odometry" MapPoint created in the last frame
                {
                    cv::rectangle(vim[c],pt1,pt2,odometryColor);
                    cv::circle(vim[c],point,2,odometryColor,-1);
                    mnTrackedVO++;
                }
            }
        }
    ...
    std::vector<cv::Mat> vimWithInfo(vim.size());
    for (int c = 0; c < vim.size(); ++c)
    {
        DrawTextInfo(vim[c],state, vimWithInfo[c]);
    }
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    if (id == 0) pTracker->mvImGray[mvIm.size()-1].copyTo(mvIm.back());
    else 
        for (int c = 0; c < mvIm.size(); ++c)
        {
            pTracker->mvImGray[c].copyTo(mvIm[c]);
        }
    mvCurrentKeys=pTracker->mCurrentFrame.mvKeys;
    mThDepth = pTracker->mCurrentFrame.mThDepth;
    mvCurrentDepth = pTracker->mCurrentFrame.mvDepth;
    mmpKey2Cam = pTracker->mCurrentFrame.mmpKeyToCam;
    ...
    mmProjectPoints = mCurrentFrame.mvmProjectPoints.back();
