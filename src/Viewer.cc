...
void Viewer::Run()
{
    ...
    while(1)
    {
        ...
        pangolin::FinishFrame();
            ...
            std::vector<cv::Mat> vim = mpFrameDrawer->DrawFrame(trackedImageScale);
            swap(vim[vim.size() - 1], vim[vim.size() - 2]);
            cv::Mat toShow = vim[0];
            ...
            for (int c = 1; c < vim.size(); c++)
            {
                cv::hconcat(toShow, vim[c], toShow);
            }
