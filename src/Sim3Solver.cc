...
Sim3Solver::Sim3Solver(MultiKeyFrame *pKF1, MultiKeyFrame *pKF2, const vector<MapPoint *> &vpMatched12, const bool bFixScale,
                       vector<MultiKeyFrame*> vpKeyFrameMatchedMP):
    mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale),
    vpCamera1(pKF1->mvpCamera), vpCamera2(pKF2->mvpCamera)
{
    // TODO:这里bDifferentKFs设置错误
    bool bDifferentKFs = false;
    // bool bDifferentKFs = true;
    if(vpKeyFrameMatchedMP.empty())
    {
        bDifferentKFs = true;
        vpKeyFrameMatchedMP = vector<MultiKeyFrame*>(vpMatched12.size(), pKF2);
    }
    ...
    Eigen::Matrix3f Rbw1 = T1bw.rotationMatrix();
    Eigen::Vector3f tbw1 = T1bw.translation();
    Eigen::Matrix3f Rbw2 = T2bw.rotationMatrix();
    Eigen::Vector3f tbw2 = T2bw.translation();
    

    for (int i = 0; i < pKF1->nCamera; ++i)
    {
        vTc1b.push_back(vT1w[i] * T1bw.inverse());
        vTc2b.push_back(vT2w[i] * T2bw.inverse());
    }
    ...
    MultiKeyFrame* pKFm = pKF2; //Default variable
    for(int i1=0; i1<mN1; i1++)
    {
        if(vpMatched12[i1])
        {
            // 匹配上当前帧原先匹配的地图点和与kf2匹配的地图点
            MapPoint* pMP1 = vpKeyFrameMP1[i1];
            MapPoint* pMP2 = vpMatched12[i1];

            int cam1 = pKF1->mmpKeyToCam[i1];
            int cam2 = 0;
            ...
            int indexKF1 = pMP1->GetIndexInKeyFrame(pKF1)[cam1];
            int indexKF2 = -1;

            vector<int> vIndicesKF2 = pMP2->GetIndexInKeyFrame(pKFm);

            for (int i = 0; i < pKFm->nCamera; ++i)
            {
                if (vIndicesKF2[i] >= 0)
                {
                    cam2 = i;
                    indexKF2 = vIndicesKF2[i];
                    break;
                }
            }
            ...
            mvX3Db1.push_back(Rbw1*X3D1w+tbw1);

            Eigen::Vector3f X3D2w = pMP2->GetWorldPos();
            mvX3Db2.push_back(Rbw2*X3D2w+tbw2);

            mvAllIndices.push_back(idx);
            mvncam1.push_back(cam1);
            mvncam2.push_back(cam2);
            idx++;
        }
    }

    FromCameraToImage(mvX3Db1,mvP1im1,vpCamera1, mvncam1, vTc1b);
    FromCameraToImage(mvX3Db2,mvP2im2,vpCamera2, mvncam2, vTc2b);
...
Eigen::Matrix4f Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)
{
    ...
    while(mnIterations<mRansacMaxIts && nCurrentIterations<nIterations)
    {
        ...
        for(short i = 0; i < 3; ++i)
        {
            ...
            P3Dc1i.col(i) = mvX3Db1[idx];
            P3Dc2i.col(i) = mvX3Db2[idx];
...
void Sim3Solver::CheckInliers()
{
    ...
    Project(mvX3Db2,vP2im1,mT12i,vpCamera1, mvncam1, vTc1b);
    Project(mvX3Db1,vP1im2,mT21i,vpCamera2, mvncam2, vTc2b);
...
void Sim3Solver::Project(const vector<Eigen::Vector3f> &vP3Dw, vector<Eigen::Vector2f> &vP2D, Eigen::Matrix4f Tbw, vector<GeometricCamera*>& vpCamera, vector<int>& vncam, vector<Sophus::SE3f>& vTcb)
{
    Eigen::Matrix3f Rbw = Tbw.block<3,3>(0,0);
    Eigen::Vector3f tbw = Tbw.block<3,1>(0,3);
    ...
    {
        int cam = vncam[i];
        Eigen::Vector3f P3Dc = vTcb[cam] * (Rbw*vP3Dw[i]+tbw);
        Eigen::Vector2f pt2D = vpCamera[cam]->project(P3Dc);
        vP2D.push_back(pt2D);
    }
}

void Sim3Solver::FromCameraToImage(const vector<Eigen::Vector3f> &vP3Db, vector<Eigen::Vector2f> &vP2D, vector<GeometricCamera*>& vpCamera, vector<int>& vncam, vector<Sophus::SE3f>& vTcb)
{
    vP2D.clear();
    vP2D.reserve(vP3Db.size());

    for(size_t i=0, iend=vP3Db.size(); i<iend; i++)
    {
        int cam = vncam[i];
        Eigen::Vector2f pt2D = vpCamera[cam]->project(vTcb[cam] * vP3Db[i]);
        vP2D.push_back(pt2D);
    }
}
