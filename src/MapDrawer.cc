...
void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph, const bool bDrawInertialGraph, const bool bDrawOptLba)
{
    ...
    const vector<MultiKeyFrame*> vpKFs = pActiveMap->GetAllKeyFrames();
    ...
    if(bDrawGraph)
    {
        ...
        {
            // Covisibility Graph
            const vector<MultiKeyFrame*> vCovKFs = vpKFs[i]->GetCovisiblesByWeight(100);
            Eigen::Vector3f Ow = vpKFs[i]->GetFrameCenter();
            if(!vCovKFs.empty())
            {
                for(vector<MultiKeyFrame*>::const_iterator vit=vCovKFs.begin(), vend=vCovKFs.end(); vit!=vend; vit++)
                {
                    if((*vit)->mnId<vpKFs[i]->mnId)
                        continue;
                    Eigen::Vector3f Ow2 = (*vit)->GetFrameCenter();
            ...
            // Spanning tree
            MultiKeyFrame* pParent = vpKFs[i]->GetParent();
            if(pParent)
            {
                Eigen::Vector3f Owp = pParent->GetFrameCenter();
                glVertex3f(Ow(0),Ow(1),Ow(2));
                glVertex3f(Owp(0),Owp(1),Owp(2));
            }

            // Loops
            set<MultiKeyFrame*> sLoopKFs = vpKFs[i]->GetLoopEdges();
            for(set<MultiKeyFrame*>::iterator sit=sLoopKFs.begin(), send=sLoopKFs.end(); sit!=send; sit++)
            {
                if((*sit)->mnId<vpKFs[i]->mnId)
                    continue;
                Eigen::Vector3f Owl = (*sit)->GetFrameCenter();
                glVertex3f(Ow(0),Ow(1),Ow(2));
    ...
    if(bDrawInertialGraph && pActiveMap->isImuInitialized())
    {
        ...
        for(size_t i=0; i<vpKFs.size(); i++)
        {
            ...
            Eigen::Vector3f Ow = pKFi->GetFrameCenter();
            MultiKeyFrame* pNext = pKFi->mNextKF;
            if(pNext)
            {
                Eigen::Vector3f Owp = pNext->GetFrameCenter();
                glVertex3f(Ow(0),Ow(1),Ow(2));
                glVertex3f(Owp(0),Owp(1),Owp(2));
            }
        }
