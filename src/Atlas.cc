...
void Atlas::AddKeyFrame(MultiKeyFrame* pKF)
{
...
std::vector<MultiKeyFrame*> Atlas::GetAllKeyFrames()
{
    unique_lock<mutex> lock(mMutexAtlas);
...
map<long unsigned int, MultiKeyFrame*> Atlas::GetAtlasKeyframes()
{
    map<long unsigned int, MultiKeyFrame*> mpIdKFs;
    for(Map* pMap_i : mvpBackupMaps)
    {
        vector<MultiKeyFrame*> vpKFs_Mi = pMap_i->GetAllKeyFrames();

        for(MultiKeyFrame* pKF_j_Mi : vpKFs_Mi)
        {
