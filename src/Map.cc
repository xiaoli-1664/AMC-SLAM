...
Map::Map():mnMaxKFid(0),mnBigChangeIdx(0), mbImuInitialized(false), mnMapChange(0), mpFirstRegionKF(static_cast<MultiKeyFrame*>(NULL)),
...
Map::Map(int initKFid):mnInitKFid(initKFid), mnMaxKFid(initKFid),/*mnLastLoopKFid(initKFid),*/ mnBigChangeIdx(0), mIsInUse(false),
                       mHasTumbnail(false), mbBad(false), mbImuInitialized(false), mpFirstRegionKF(static_cast<MultiKeyFrame*>(NULL)),
...
void Map::AddKeyFrame(MultiKeyFrame *pKF)
{
...
void Map::EraseKeyFrame(MultiKeyFrame *pKF)
{
    unique_lock<mutex> lock(mMutexMap);
    ...
    if(mspKeyFrames.size()>0)
    {
        if(pKF->mnId == mpKFlowerID->mnId)
        {
            vector<MultiKeyFrame*> vpKFs = vector<MultiKeyFrame*>(mspKeyFrames.begin(),mspKeyFrames.end());
...
vector<MultiKeyFrame*> Map::GetAllKeyFrames()
{
    unique_lock<mutex> lock(mMutexMap);
    return vector<MultiKeyFrame*>(mspKeyFrames.begin(),mspKeyFrames.end());
}
...
//        delete *sit;
    ...
    for(set<MultiKeyFrame*>::iterator sit=mspKeyFrames.begin(), send=mspKeyFrames.end(); sit!=send; sit++)
    {
...
void Map::ApplyScaledRotation(const Sophus::SE3f &T, const float s, const bool bScaledVel)
{
    ...
    for(set<MultiKeyFrame*>::iterator sit=mspKeyFrames.begin(); sit!=mspKeyFrames.end(); sit++)
    {
        MultiKeyFrame* pKF = *sit;
...
void Map::PreSave(std::set<GeometricCamera*> &spCams)
{
    ...
    for(MapPoint* pMPi : mspMapPoints)
    {
        ...
        map<MultiKeyFrame*, std::vector<int>> mpObs = pMPi->GetObservations();
        for(map<MultiKeyFrame*, std::vector<int>>::iterator it= mpObs.begin(), end=mpObs.end(); it!=end; ++it)
        {
    ...
    for(MultiKeyFrame* pKFi : mspKeyFrames)
    {
        if(!pKFi || pKFi->isBad())
            continue;
...
void Map::PostLoad(KeyFrameDatabase* pKFDB, ORBVocabulary* pORBVoc/*, map<long unsigned int, KeyFrame*>& mpKeyFrameId*/, map<unsigned int, GeometricCamera*> &mpCams)
{
    ...
    map<long unsigned int, MultiKeyFrame*> mpKeyFrameId;
    for(MultiKeyFrame* pKFi : mspKeyFrames)
    {
        if(!pKFi || pKFi->isBad())
            continue;
