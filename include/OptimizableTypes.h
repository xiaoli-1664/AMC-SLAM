...
public:
    ...
    std::vector<GeometricCamera*> vpCamera1;
    std::vector<GeometricCamera*> vpCamera2;
...
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeSim3ProjectXYZ(int cam1, int cam2, const std::vector<g2o::SE3Quat>& vTc1b, const std::vector<g2o::SE3Quat>& vTc2b)
                            : cam1_(cam1), cam2_(cam2), vTc1b_(vTc1b), vTc2b_(vTc2b) {}
    ...
    void computeError()
    {
        ...
        Eigen::Vector2d obs(_measurement);
        _error = obs-v1->vpCamera1[cam1_]->project(vTc1b_[cam1_].map(v1->estimate().map(vTc2b_[cam2_].inverse().map(v2->estimate()))));
    }
...
public:
    int cam1_;
    int cam2_;
    std::vector<g2o::SE3Quat> vTc1b_;
    std::vector<g2o::SE3Quat> vTc2b_;
};
...
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    EdgeInverseSim3ProjectXYZ(int cam1, int cam2, const std::vector<g2o::SE3Quat>& vTc1b, const std::vector<g2o::SE3Quat>& vTc2b)
                            : cam1_(cam1), cam2_(cam2), vTc1b_(vTc1b), vTc2b_(vTc2b) {}
    virtual bool read(std::istream& is);
    virtual bool write(std::ostream& os) const;
    ...
    void computeError()
    {
        ...
        Eigen::Vector2d obs(_measurement);
        _error = obs-v1->vpCamera2[cam2_]->project((
                            vTc2b_[cam2_].map(v1->estimate().inverse().map(vTc1b_[cam1_].inverse().map(v2->estimate())))));
    }
